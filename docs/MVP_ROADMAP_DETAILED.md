...
- **Backend**
  - Завершить авто-запуск миграций (готово) и добавить REST `GET /admin/migrations/status` с полями `applied`, `pending`, `lastAppliedAt`.
  - Расширить логирование `/auth/telegram`: сохранять причину отказа, длину initData, используемый botToken, user-id Telegram (без PII).
  - Добавить транзакционный healthcheck `GET /admin/health/full`, агрегирующий DB, Redis и миграции.
- **Frontend**
  - В `initGame` внедрить очередь запросов: один активный POST + повтор с экспоненциальной паузой до 3 попыток.
  - Отрисовывать модалку с пояснением при ошибке авторизации и ручной кнопкой «Повторить».
- **Data / QA**
  - Составить чек-лист трассировки: какой лог где искать (auth flow, миграции, Redis).
  - Набросать ручные сценарии для авторизации (валидный initData, просроченный, повреждённый).
- **Ops**
  - Документировать параметры окружения Railway (переменные, secrets) в `docs/STATUS.md` для быстрой диагностики.

**Should Have (MoSCoW: Should)**  
- Скрипт начальной инициализации БД для локалки (`scripts/bootstrap-db.ts`) с прогоном миграций и сидов.
- Мини-тест на бэке, проверяющий ответ `/admin/migrations/status` (Jest + supertest).
...
**Must Have**
- **Backend**
  - Ввести агрегацию тапов: Redis-буфер (ключ `tap:{userId}`) + воркер сброса в PostgreSQL каждые 500мс или при превышении 50 тапов.
  - Создать таблицу `tap_events` (id, user_id, taps, energy_delta, created_at) с индексом по `user_id`.
  - Пересчитать выдачу оффлайн-дохода: хранить timestamp последнего сброса, вернуть в `/session`.
- **Frontend**
  - Реализовать streak-систему (визуальный счётчик + крит-анимация на N тап).
  - Переписать панель построек: отображать пассивный доход в реальном времени (подставлять данные из `/session` + апдейт на каждую покупку).
- **Content**
  - Баланс Tier1–Tier3: пересчитать cost/profit, обновить `content/buildings.json`, добавить таблицу сравнения.
- **QA/Data**
  - Логировать события `tap_batch_commit`, `building_purchase`, `offline_income_grant` (минимальные payload: userId, величина, источник).

**Should Have**
- Экран «Итоги сессии» после возврата из офлайна с детализацией: энергия, XP, бусты.
- Авто-добавление первой солнечной панели, если игрок пропустил обучение.
