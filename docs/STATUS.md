...
- **Buildings UI**: вкладка «Постройки» в клиенте (на `/session` приходят `inventory`), можно покупать/апгрейдить; бэкенд `UpgradeService` в ответах возвращает актуальные доходы/стоимость.
