...
- **Чек-лист:**
  ...
  2.  Добавить REST `/api/v1/referrals` (claim, stats) + таблицы `referral_codes`, `referral_events`.
  3.  Обновить UI: экран «Пригласи друзей», deeplink генератор, промокоды в onboarding.
  4.  Подготовить экспорт в маркетинговую CRM (csv/webhook) для трекинга инфлюенсеров.
  5.  Телеметрия: `referral_share_click`, `referral_activation`, `referral_purchase`.
...
3. Подготовить старт Stage 3: финализировать список экспериментов и пуш-кампаний, оценить нагрузку на команду и забронировать время аналитика/DevOps.
