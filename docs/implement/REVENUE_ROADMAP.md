...
_Статус: ⏳ не начат — стартуем после подтверждения целевых Stage 2 (D1 ≥35%, D7 ≥12%, ARPDAU +15%)._

### 3.1 A/B инфраструктура
- **Ответственные:** backend — команда API, frontend — команда webapp, контент — гейм-дизайнер.
- **Чек-лист (Т+0…Т+6 недель):**
  1.  Перевести ценовые/контентные конфиги в версионируемый формат (`content/experiments/*.yaml`) с поддержкой fallback.
  2.  Реализовать сервис `ExperimentService` в backend: детерминированное распределение по userId, хранение варианта в Redis + PG (`experiments_assignments`).
  3.  Добавить middleware для прокидывания варианта в ответ `/session`, расширить `gameStore` для чтения флага.
  4.  Поддержать feature-брейкеры в UI (Shop, Boost Hub, MainScreen) и завершить telemetry (`experiment_assignment`/`experiment_exposure`).
  5.  Подготовить CLI `scripts/experiments.ts` + админку для включения/отключения варианта, QA + документация (docs/analytics/EXPERIMENT_PLAYBOOK.md).
- **Зависимости:** Redis (готов), потребуются новые миграции и контент-файлы.

### 3.2 Telegram-ориентированные кампании
- **Ответственные:** продакт + маркетинг, поддержка backend (webhook) и DevOps (очереди).
- **Чек-лист:**
  1.  Сконфигурировать bot API: отдельный токен, таблица `telegram_push_queue` + job consumer (BullMQ) с ретраями.
  2.  Описать сценарии сообщений (`docs/marketing-analysis/telegram_flows.md`): daily boost reminder, prestige nudge, quest unclaimed, seasonal event kick-off.
  3.  Реализовать эндпоинт `/api/v1/notifications/telegram` с подписью и rate-limit.
  4.  Настроить отправку по расписанию (cron в worker) и добавить телеметрию `telegram_push_sent/delivered/click`.
  5.  Подготовить дашборд «Push Uplift» (open rate, CTR, impact на DAU).

### 3.3 Influencer & referral
- **Ответственные:** Growth (партнёры), backend/frontend для реализации механик.
- _Статус: ✅ MVP в проде — базовый флоу и UI готовы, предстоит масштабирование на каналы и партнёров._
- **Чек-лист:**
  1.  ✅ Специфицировать реферальные награды (Stars + косметика) и описать в `content/referrals.json`.
  2.  ✅ Добавить REST `/api/v1/referrals` (summary/activate/milestones) + таблицы `referral_codes`, `referral_relations`, `referral_rewards`, `referral_events`.
  3.  ✅ Обновить UI: панель «Друзья и награды» в настройках, копирование/шаринг, ввод чужого кода.
  4.  🔄 Подготовить экспорт в маркетинговую CRM (csv/webhook) для трекинга инфлюенсеров.
  5.  ✅ Телеметрия: `referral_summary_loaded`, `referral_code_activated`, `referral_milestone_claim`.

### 3.4 LTV vs CAC контроль
- **Ответственные:** продуктовый аналитик + финансы.
- **Чек-лист:**
  1.  Свести модель LTV (Sheets + SQL): когортный расчёт на основе данных `purchases`, `events`.
  2.  Определить CAC по каналам (органика, рекламные кампании, инфлюенсеры) и настроить еженедельный отчёт в `docs/analytics/ltv_vs_cac.md`.
  3.  Согласовать допустимые payback-пороги (например, payback ≤ 45 дней) и внедрить алерты при отклонениях.
  4.  Интегрировать KPI в админский экран: добавить виджет «Budget Guardrail» (берём данные из отчётов).
...
## Следующие шаги
1. Прокатить cron-сброс квестов на dev/stage, снять метрики `quest_reset_scheduler_tick` и убедиться, что baseline корректно обновляется (до 30 октября 2025).  
2. Собрать отчёт по конверсиям новых CTA (импрессии/клики из `leaderboard_shop_cta_*`, `monetization_prompt_*`, `daily_boost_upsell_*`) и утвердить лимиты показа с продактом.  
3. Развернуть реферальный дашборд (`referral_*` таблицы + события) и подготовить CRM-экспорт/фильтры для инфлюенсеров.  
4. Финализировать старт Stage 3: список экспериментов, сценарии Telegram-пушей, оценка нагрузки на аналитика и DevOps.
