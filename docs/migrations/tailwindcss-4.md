...
3. **Перенести кастомные токены.** Наш файл `src/styles/design-tokens.css` останется в силе, но нужно проверить, не дублируем ли значения с обновлёнными встроенными токенами (Tailwind 4 добавил поддержку CSS custom properties из коробки). Возможно, часть цветов и spacing можно удалить из `extend` и заменить переменными.
