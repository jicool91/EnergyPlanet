  ...
  return (
    <div className="p-0 flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-xs text-white/55">Всего игроков: {leaderboardTotal.toLocaleString()}</span>
      </div>
      <div className="rounded-lg overflow-hidden bg-dark-secondary/60 border border-cyan/[0.12] backdrop-blur-[6px]">
        <table className="w-full border-collapse text-[13px]">
          <thead>
            <tr>
              <th className="px-[14px] py-3 text-left border-b border-white/[0.04] font-semibold text-white/70 text-xs uppercase">#</th>
              <th className="px-[14px] py-3 text-left border-b border-white/[0.04] font-semibold text-white/70 text-xs uppercase">Игрок</th>
              <th className="px-[14px] py-3 text-left border-b border-white/[0.04] font-semibold text-white/70 text-xs uppercase">Уровень</th>
              <th className="px-[14px] py-3 text-left border-b border-white/[0.04] font-semibold text-white/70 text-xs uppercase">Энергия</th>
            </tr>
          </thead>
          <tbody>
            {rows.map(entry => (
              ...
              >
                ...
                <td className="px-[14px] py-3 text-left">
                  <div className="flex flex-col gap-[2px]">
                    <span className="font-semibold">{entry.username || entry.first_name || 'Игрок'}</span>
                    <span className="text-[11px] text-white/40">#{entry.user_id.slice(0, 6)}</span>
                  </div>
                </td>
                <td className="px-[14px] py-3 text-left">{entry.level}</td>
                <td className="px-[14px] py-3 text-left">{Math.floor(entry.total_energy_produced).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
